$(function() {
	...
	function getWeather() {
		$.ajax({
			...
			dataType: 'json',
			success: (data) => {
				let currentTemp = Math.round(data.current_observation.temp_c);
				...
				$('.temp-switch-checkbox').change(function() {
					...
					for(let i = 0; i < 5; i++) {
						let castDeg = Math.round(data.forecast.simpleforecast.forecastday[i].high.celsius);
						$(this).is(':checked') ? $(`.forecast-deg:eq(${i})`).html(`${castDeg}&deg;C`) : $('.forecast-deg:eq(' + i + ')').html(`${Math.round(castDeg*9/5+32)}&deg;F`);
					}
				});

				// Add class to show weather forecast icon
				let iconSet  = ['clear-sun', 'cloudy', 'flurries-sleet', 'fog-hazy', 'm-cloud-p-sun', 'm-sun-p-cloud', 'snow', 'rain', 'tstorms', 'flurries-sleet-chance', 'rain-chance', 'snow-chance',	'tstorms-chance',	'unknown'];
				for (let i = 0; i < 5; i++) {
					let icon = $(`.forecast-icon:eq(${i})`),
							iconName = data.forecast.simpleforecast.forecastday[i].icon;
					switch (iconName) {
						case ('clear' || 'sunny'):
    ...
    checker = parseInt(hrsNmins);
		...
		if (checker > 2100 || checker < 500) { $('body').addClass('night-bg') }

    if (checker >= 500 && checker <= 930) { $("#dynamic-welcome").html("Rise and Shine"); }
    else if (checker > 930 && checker < 1200) { $("#dynamic-welcome").html("Good Morning"); }
    else if (checker >= 1200 && checker < 1700) { $("#dynamic-welcome").html("Good Afternoon"); }
    else if (checker >= 1700 && checker < 2100) { $("#dynamic-welcome").html("Good Evening"); }
    else if (checker >= 2100 || (checker >= 0 && hrsNmins <= 59)) { $(".welcome").html("Having a Nice Night?"); }
    else if (checker >= 100 && checker < 500) { $("#dynamic-welcome").html("You Should Be Asleep!"); }
  };

	// Get next 5 days names
	(() => {
		// Create an array storing day names
		let DayAsString = dayIndex => {
	...
	setInterval(() =>{
		let heightNow = $(window).outerHeight(true);
